...
probeset2pathwayTrain <-function(x=x, y=y, inter, pathSet)
{
  ...
  #for KEGG.db
  # KEGGids = mget(substring(inter,2),KEGGEXTID2PATHID,ifnotfound=NA)
  # KEGGids = KEGGids[which(KEGGids!="NA")] # 6909 genes to pathways
  # ##
  # kidList <- unlist(KEGGids)
  # kidList <- kidList[which(kidList!="NA")] # 24349 genes to pathways
  # ukidList <-unique(kidList)
  #
  ## form probset belond to KeggID,
  #nCol = length(ukidList)	# 227 pathways mapped
  nCol = length(pathSet)
  nRow = nrow(x)
  nColx = ncol(x)
  ap <- matrix(0, ncol=nCol, nrow=nRow) # pathway profile 465 samples x 227 pathways
  ...
  for (i in 1 : length(pathSet))
  {
    ...
    if (length(Vpathwayi) > 0){
      for (j in 1 : length(Vpathwayi)){
        index <- which(substring(colnames(x),2)==Vpathwayi[j])
        len.k=length(index)
        kk=1
        if(len.k==0)
        {
        ...
        else if(len.k >= 1)
        {
          ...
          for(ii in 1: len.k)
            tTest[ii] = abs(stats[colnames(x)[index[ii]],"stat"])
          ...
          if(len.k>=2)
          {
            ...
            netscore = abs(stats[colnames(x)[index[tTest.order[1]]],"stat"])
            for(kk in 2: len.k)
            {
